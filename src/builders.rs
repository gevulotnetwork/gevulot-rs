...
impl MsgCreateWorkflowBuilder {
    pub fn into_message(&self) -> Result<gevulot::MsgCreateWorkflow> {
        ...
        Ok(gevulot::MsgCreateWorkflow {
            creator: msg.creator,
            spec: Some(gevulot::WorkflowSpec {
                stages: msg.stages.into_iter().map(|stage| gevulot::workflow_spec::Stage {
                    tasks: stage.tasks.into_iter().map(|task| gevulot::TaskSpec {
                        image: task.image,
                        command: task.command,
                        args: task.args,
                        env: task.env.into_iter().map(|e| gevulot::TaskEnv {
                            name: e.name,
                            value: e.value,
                        }).collect(),
                        input_contexts: task.input_contexts.into_iter().map(|ic| gevulot::InputContext {
                            source: ic.source,
                            target: ic.target,
                        }).collect(),
                        output_contexts: task.output_contexts.into_iter().map(|oc| gevulot::OutputContext {
                            source: oc.source,
                        ...
                        time: task.resources.time,
                        store_stdout: task.store_stdout.unwrap(),
                        store_stderr: task.store_stderr.unwrap(),
